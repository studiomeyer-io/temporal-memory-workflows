...
async function run() {
  ...
  if (memory instanceof InMemoryMemoryClient) {
    console.warn(
      "[worker] No NEX_MEMORY_API_KEY set — using in-memory client. Decisions will NOT persist across worker restarts.",
    );
  }
  ...
  // @temporalio/worker installs a default SIGTERM handler internally; the explicit
  // handlers below make the intent visible at the call site so future contributors
  // know shutdown is wired up.
